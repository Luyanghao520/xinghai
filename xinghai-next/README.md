...
| `/contact` | 联系我们（占位） |
| `/admin` | 后台看板：报名统计/搜索 + 招新申请列表 + 成员概览（只读，需口令） |
...
- 能力：报名总数/今日新增统计、按意向方向统计、报名列表（姓名/手机号/方向/院系模糊搜索）、
  招新申请列表（含待审统计）、成员/校友概览；
- 会话为 HttpOnly + SameSite=Strict 的 Cookie（8 小时），值为口令摘要而非明文；
- 审核类写操作（通过/驳回申请、录取入会）暂在旧栈后台进行，属后续迭代。
...
- [ ] **导入真实数据**：PythonAnywhere 导出 3 库 → `npm run migrate`（管道已验证）
- [ ] **报名体验完善**：意向方向改为后台可配置的选项、提交后确认页、（可选）邮件/短信通知
- [ ] **后台写操作**：申请审核（通过/驳回）、报名录取入会、数据导出 CSV（对齐旧栈 admin.html）
- [ ] **内容填充**：首页文案、社团介绍、演出作品数据化、联系方式
- [ ] **视觉设计**：替换占位色板与字体、汉堡图标、动效；引入 shadcn/ui（可选）
- [ ] **SEO/分享**：sitemap、robots、OG 图（旧栈首页三件套经验可平移）
- [ ] **正式上线**：平台选型确认后绑定域名、配置 HTTPS 与监控、旧链接 301
